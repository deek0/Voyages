...
voyage_data = pd.read_csv('voyages2.csv')
...
def get_population_histogram():
    ...
    # score is maxed at 0.3 if less than
    ...
    for index,item in grouped_by_ports.items():
        ...
        if total_voyages >4:
            ...
            predict_score = min(count/total_voyages, 0.3)
...
def find_starting_voyages_test():
    ...
    return starting_voyages
# print(voyage_data.groupby(['vessel', 'begin_port_id', 'end_port_id']).size().to_string())
# print(voyage_data.groupby(['vessel', 'begin_port_id', 'end_port_id']).size().groupby(['vessel', 'begin_port_id']).sum().to_string())
# print(get_population_histogram())
# print(get_vessel_hist(1))
...
# print(grouped_by_ports.groupby('begin_port_id').sum().get(7))
